...
import (
	...
	"github.com/ymiz/go-cxac/bitflyer/websocket/jsonrpc/subscription/channel/execution"
	"log"
	"os"
	"os/signal"
)
...
func Example() {
	...
	client := NewClient(c, public.EndPoint, subscription.Parameter{
		Version: "2.0",
		Method:  "subscribe",
		Params:  subscription.Params{Channel: execution.Generate(product.FxBtcJpy)},
	...
	for {
		...
		case <-interrupt:
			return
		case e := <-c:
			log.Println("example bf execution client", e)
		}
	}
}
